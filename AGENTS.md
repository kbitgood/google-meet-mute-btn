...
A global keyboard shortcut utility for macOS that toggles mute in ALL Google Meet tabs across Arc browser windows, regardless of which application is currently focused.
...
2. The script bulk-fetches all tab URLs per Arc window (fast)
3. Finds Meet tabs by URL, then targets them by index (workaround for Arc's buggy tab iteration)
4. Dispatches `Cmd+D` keyboard event via JavaScript to toggle mute
5. Plays a sound to indicate the new mute state (Morse=muted, Pop=unmuted, Basso=error)
...
| Location | Purpose |
|----------|---------|
| `~/.local/bin/toggle-meet-mute.sh` | Main toggle script (bash wrapper) |
| `~/.local/bin/toggle-meet-mute.scpt` | AppleScript that does the actual work |
...
4. Test with Arc browser specifically (not Chrome)
